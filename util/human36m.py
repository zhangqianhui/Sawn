import numpy as np
import copy
from util.camera import normalize_screen_coordinates
...
def init_position_data(path):
    file_data = np.load(path, allow_pickle=True)
    if 'positions_2d' in file_data:
        file_data = file_data['positions_2d'].item()
        data_type='2d'
    elif 'positions_3d' in file_data:
        file_data = file_data['positions_3d'].item()
        data_type='3d'
    data = {}
    for subject, actions in file_data.items():
        data[subject] = {}
        for action_name, positions in actions.items():
            if data_type == '2d':
                data[subject][action_name]={}
                for i in range(len(positions)):
                    data[subject][action_name][str(i)]=positions[i]
            elif data_type == '3d':
                data[subject][action_name] = positions
    return data, data_type

def init_cameras_param():
    cameras_dic = copy.deepcopy(h36m_cameras_extrinsic_params)

    for cameras in cameras_dic.values():
...
def init_video_list(path_input, path_gt, exclude_subject_list):
    file_data_input = np.load(path_input, allow_pickle=True)['positions_2d'].item()
    file_data_gt    = np.load(path_gt,    allow_pickle=True)
    if 'positions_2d' in file_data_gt:
        file_data_gt = file_data_gt['positions_2d'].item()
    elif 'positions_3d' in file_data_gt:
        file_data_gt = file_data_gt['positions_3d'].item()

    data_list=[]
    subject_input = file_data_input.keys()
    subject_gt    = file_data_gt.keys()
    for subject in subject_input:
        if subject in subject_gt and subject not in exclude_subject_list:
            action_input = file_data_input[subject].keys()
            action_gt    = file_data_gt[subject].keys()
            for action in action_input:
                if action in action_gt:
                    for camera in range(len(file_data_input[subject][action])):
                        name={}
...
def check_dataset(data_gt, data_input, data_list, gt_data_type):
    out_lists = []
    for seq_name in data_list:
        if gt_data_type == '2d':
            video_gt = data_gt[seq_name['subject']][seq_name['action']][seq_name['camera']]
        else:
            video_gt = data_gt[seq_name['subject']][seq_name['action']]

        video_input = data_input[seq_name['subject']][seq_name['action']][seq_name['camera']]
        ratio = np.abs(video_gt.shape[0]-video_input.shape[0])/video_gt.shape[0]
        if ratio>0.01:
            pass
        else:
            out_lists.append(seq_name)
    return out_lists
