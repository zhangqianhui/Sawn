...
def find_network_using_name(opt, target_network_name, filename, extend=None):
    extend=filename if extend is None else extend
    if opt.is_texturet and 'generat' in filename:
        target_class_name = target_network_name + extend + 'Texture'
    else:
...
def define_g(opt, name=None, filename=None, **parameter_dic):
    ...
    netG_cls = find_network_using_name(opt, name, filename, extend='generator')
...
def define_d(opt, name=None, filename=None, **parameter_dic):
    ...
    netD_cls = find_network_using_name(opt, name, filename)
...
def define_e(opt, name=None, **parameter_dic):
    ...
    netE_cls = find_network_using_name(opt, name, 'encoder')
